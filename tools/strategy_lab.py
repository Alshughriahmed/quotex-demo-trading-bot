...
def classify_row(row: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if losses > closed * 0.45 and closed > 0:
        notes.append("loss count is high")

    if consistency >= 55 and win_rate >= 65 and closed >= 30 and worst > 0:
        return "PROMISING", notes or ["balanced across files"]
...
def build_markdown_report(data: dict[str, Any], top: int) -> str:
    ...
    lines.append("")
    lines.append("| Rank | Decision | Duration | Min confidence | Lookback | Trades | Win rate | Losses | Worst score | Consistency | Notes |")
    lines.append("|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---|")
