...
def load_rows(db: sqlite3.Connection) -> list[ReplayRow]:
    rows = db.execute(
        """
        SELECT s.source_key, s.asset, s.direction,
               COALESCE(s.entry_time, s.signal_time) AS event_time,
               o.result, COALESCE(o.theoretical_profit_loss, 0) AS profit_loss
        FROM research_signals s
        JOIN research_signal_outcomes o ON o.signal_id = s.id
        WHERE s.direction IN ('CALL', 'PUT')
        ORDER BY s.source_key, s.asset, COALESCE(s.entry_time, s.signal_time), s.id
        """
    ).fetchall()
...
def candle_counts(db: sqlite3.Connection) -> dict[tuple[str, str, str], int]:
    ...
    return {(str(r["source_key"] or "unknown"), str(r["asset"] or "unknown"), str(r["month"] or "unknown")): int(r["candles"] or 0) for r in rows}


def group_rows(rows: list[ReplayRow]) -> dict[tuple[str, str, str], list[ReplayRow]]:
    grouped: dict[tuple[str, str, str], list[ReplayRow]] = defaultdict(list)
    for row in rows:
        grouped[(row.source_key, row.asset, row.month)].append(row)
    return dict(grouped)
...
def label(key: tuple[str, str, str] | None) -> str:
    if key is None:
        return "n/a"
...
def build_reports(grouped: dict[tuple[str, str, str], list[ReplayRow]], rule_set: list[Rule]) -> tuple[dict[str, dict[tuple[str, str, str], dict[str, object]]], list[dict[str, object]]]:
    by_rule: dict[str, dict[tuple[str, str, str], dict[str, object]]] = {rule.name: {} for rule in rule_set}
    ...
    for rule in rule_set:
        tested = {k: m for k, m in by_rule[rule.name].items() if int(m["trades"]) > 0}
        months = len(tested)
        ...
        one_month = (months == 1 and profit > 0) or (profit > 0 and positive_sum > 0 and best_profit / positive_sum >= ONE_MONTH_DEPENDENCY_RATIO)
        ...
        stability.append(
            {
                ...
                "best_month": label(best),
                "best_pl": fnum(best_profit),
                "worst_month": label(worst),
                "worst_pl": fnum(worst_profit),
                "one_month_dependency": "YES" if one_month else "NO",
                "small_sample": "YES" if small_sample else "NO",
                "status": status,
            }
        )
    order = {"RESEARCH_KEEP": 0, "WATCH_ONLY": 1, "REJECT_NOW": 2}
    stability.sort(key=lambda r: (order.get(str(r["status"]), 9), -float(str(r["profit_loss"]))))
...
def main() -> int:
    ...
    with connect_read_only() as db:
        ...
        print(f"Detected replay groups: {len(set(grouped) | set(candles))}")
        ...
        section("Detected replay groups")
        group_table = []
        for key in sorted(set(grouped) | set(candles)):
            group_table.append({"source_key": key[0], "asset": key[1], "month": key[2], "candles": candles.get(key, 0), "outcomes": len(grouped.get(key, []))})
        table(["source_key", "asset", "month", "candles", "outcomes"], group_table)
        ...
        for key in sorted(grouped):
            print()
            print(f"Group: {label(key)}")
            rows_out = []
            for rule in rule_set:
                m = by_rule[rule.name][key]
                rows_out.append({"rule": rule.name, "trades": m["trades"], "wins": m["wins"], "losses": m["losses"], "draws": m["draws"], "unknown": m["unknown"], "win_rate": fpct(float(m["win_rate"])), "profit_loss": fnum(float(m["profit_loss"])), "avg_profit": fnum(float(m["avg_profit"])), "status": m["status"]})
            table(["rule", "trades", "wins", "losses", "draws", "unknown", "win_rate", "profit_loss", "avg_profit", "status"], rows_out)

        section("Rule stability across months")
        table(["rule", "months", "win_months", "loss_months", "flat_months", "trades", "win_rate", "profit_loss", "best_month", "best_pl", "worst_month", "worst_pl", "one_month_dependency", "small_sample", "status"], stability)
        print()
        print("Status note: RESEARCH_KEEP means research-only. Not a trading permission.")
        print("Small sample thresholds: total trades < 100 or any tested month < 15 trades.")
        print("One-month dependency threshold: best positive month >= 70% of positive monthly profit.")
        ...
        section("Conclusion")
        print(f"Replay groups evaluated: {len(grouped)}")
        if len(grouped) < 2:
            print("Only one source/month group is available. Positive rules are still single-month hypotheses.")
            print("Next step: add another USD/CAD M1 month and rerun this report before trusting any filter.")
        else:
            print("Use the stability table, not only the total profit, to judge each rule.")
            print("A rule that wins overall but loses a month, depends on one month, or has a small sample remains WATCH_ONLY.")
        ...
        print(f"RESEARCH_KEEP rules: {len(keep)}")
        print(f"WATCH_ONLY rules: {len(watch)}")
        print(f"REJECT_NOW rules: {len(reject)}")
        if keep:
            print("Research-only candidates:")
            for row in keep[:5]:
                print(f"- {row['rule']}: P/L {row['profit_loss']} over {row['trades']} trades. Research only. Not a trading permission.")
        elif watch:
            print("No rule is strong enough for RESEARCH_KEEP yet. Best WATCH_ONLY rules need more out-of-sample months.")
        else:
