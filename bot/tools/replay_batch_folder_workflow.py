...
RAW_PATTERN = re.compile(r"^DAT_ASCII_([A-Z]{6})_M(\d+)_(20\d{4})\.csv$", re.IGNORECASE)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Automated local replay research workflow for all raw HistData CSV files in external_inputs.")
    parser.add_argument("--symbol", default="", help="Optional compact symbol filter, e.g. USDCAD or EURUSD.")
    parser.add_argument("--month", default="", help="Optional month filter, e.g. 202601 or 2026-01.")
...
def compact_month(value: str) -> str:
    text = str(value or "").replace("-", "").strip()
    return text


def discover_files(symbol_filter: str, month_filter: str, limit_files: int) -> list[RawFile]:
    symbol_filter = symbol_filter.replace("/", "").upper().strip()
    month_filter = compact_month(month_filter)
    files: list[RawFile] = []
    for path in sorted(INBOX_DIR.glob("DAT_ASCII_*_M*_20*.csv")):
        ...
        if symbol_filter and symbol != symbol_filter:
            continue
        if month_filter and month_raw != month_filter:
            continue
        asset = DEFAULT_ASSET_MAP.get(symbol, f"{symbol[:3]}/{symbol[3:]}")
        timeframe_seconds = timeframe_from_m(timeframe_minutes)
        source_key = f"histdata_{symbol.lower()}_m{timeframe_minutes}_{month_raw}"
...
def main() -> int:
    ...
    print(f"Month filter: {args.month.strip() or 'ALL'}")
    ...
    for raw in files:
        print(f"- {raw.path.name} -> {raw.asset} / {raw.source_key}")
