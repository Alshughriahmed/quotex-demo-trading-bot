...
echo.
echo [4/7] Data quality report
cd /d "%~dp0bot"
python tools\data_quality_report.py
if errorlevel 1 goto error

echo.
echo [5/7] Market source status
python tools\market_source_status.py
if errorlevel 1 goto error

echo.
echo [6/7] External dataset inventory
python tools\external_inventory.py
if errorlevel 1 goto error

echo.
echo [7/7] Security safety scan
cd /d "%~dp0"
python bot\tools\safety_check.py
if errorlevel 1 goto error
